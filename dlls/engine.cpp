...
edict_t* pfnFindEntityByString(edict_t *pEdictStartSearchAfter, const char *pszField, const char *pszValue)
{
	if (FStrEq(pszValue, "info_map_parameters") && (mod_id == CSTRIKE_DLL)){
		...
		switch(g_iTypeoM){
		case MT_AS:
			fM[TE] = RANDOM_FLOAT(-1,jb_campprobability->value/1.5);
			fM[CT] = RANDOM_FLOAT(-1,jb_campprobability->value);
			break;
		case MT_DE:
			fM[TE] = RANDOM_FLOAT(-1,jb_campprobability->value);
			fM[CT] = RANDOM_FLOAT(-1,jb_campprobability->value/1.2);
			break;
		case MT_CS:
		default:
			fM[TE] = RANDOM_FLOAT(-1,jb_campprobability->value/1.5);
